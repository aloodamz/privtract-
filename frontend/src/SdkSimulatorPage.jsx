...
// Synchronous standard cryptographic SHA-256 algorithm
function sha256_sync(ascii) {
  ...
  const words = [];
  const asciiLength = ascii.trim().length;
  for (let i = 0; i < asciiLength; i++) {
    words[i >> 2] |= (ascii.trim().charCodeAt(i) & 0xff) << (24 - (i % 4) * 8);
  }
  
  const maxWords = ((asciiLength + 8) >> 6) + 1 << 4;
...
export default function SdkSimulatorPage() {
      ...
      check: (tx, cfg, state) => {
        const isWhitelisted = cfg.wallet.recipient_whitelist.some(addr => {
          if (tx.to_mode === "sha256") {
            ...
            // Generates hash of whitelist item on the fly for verification
            const hashVal = "sha256:" + CryptoJS_mock_sha256(addr);
            return hashVal === tx.to;
          } else if (tx.to_mode === "blake3") {
            // Compare BLAKE3 hashes
            const hashVal = "blake3:" + CryptoJS_mock_blake3(addr);
            return hashVal === tx.to;
          } else {
            return addr === tx.to;
          }
      ...
      check: (tx, cfg, state) => {
        ...
        for (const [id, agent] of Object.entries(cfg.agents)) {
          if (tx.from_agent_mode === "sha256") {
            const hashVal = "sha256:" + CryptoJS_mock_sha256(id);
            if (hashVal === tx.from_agent) {
              matchedAgentId = id;
              break;
            }
          } else if (tx.from_agent_mode === "blake3") {
            const hashVal = "blake3:" + CryptoJS_mock_blake3(id);
            if (hashVal === tx.from_agent) {
              matchedAgentId = id;
              break;
            }
  ...
  // Helper algorithms to mock hashing synchronously in mapping iterations
  function CryptoJS_mock_sha256(str) {
    return sha256_sync(str);
  }

  // Salted mock for blake3
  function CryptoJS_mock_blake3(str) {
    return sha256_sync("blake3-salt-key-928374982734982:" + str);
  }
